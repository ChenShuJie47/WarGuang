...
static func take_damage_with_type(player: Node, damage_source_position: Vector2, damage: int = 1, damage_type: int = DAMAGE_NORMAL, knockback_force: Vector2 = Vector2.ZERO) -> void:
	if _try_consume_backstep_counter(player):
		return
...
static func _try_consume_backstep_counter(player: Node) -> bool:
	if not player.has_method("is_backstep_counter_window"):
		return false
	if not player.is_backstep_counter_window():
		return false

	if player.has_method("consume_backstep_counter"):
		player.consume_backstep_counter()

	var preset: String = "heavy"
	var preset_value: Variant = player.get("backstep_counter_slow_preset")
	if typeof(preset_value) == TYPE_STRING:
		preset = String(preset_value)

	if TimerControlManager and TimerControlManager.has_method("start_counter_slow_motion_for_player"):
		TimerControlManager.start_counter_slow_motion_for_player(player, preset)
	elif TimerControlManager and TimerControlManager.has_method("slow_motion"):
		TimerControlManager.slow_motion(preset)

	player.trigger_feedback_event(&"backstep_counter", {
		"preset": preset,
