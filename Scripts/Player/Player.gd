...
enum PlayerState {
	...
	SUPERDASHSTART, SUPERDASH, BACKSTEP
}
## 伤害类型枚举
enum DamageType {
	NORMAL,          # 普通伤害
...
var door_autowalk_jump_phase: int = 0           # Door 自动走位跳跃阶段（0=接近，1=垂直起跳，2=抛物线）
...
var backstep_unlocked: bool = false              # 标记后撤步能力是否已解锁
...
var counter_slow_visual_overlay_active: bool = false
var counter_slow_modulate_backup: Color = Color(1, 1, 1, 1)
var counter_slow_modulate_applied: bool = false
var counter_slow_room_modulate_target: CanvasItem = null
var counter_slow_room_modulate_backup: Color = Color(1, 1, 1, 1)
var counter_slow_room_modulate_applied: bool = false
var counter_slow_room_modulate_tween: Tween = null
var counter_slow_canvas_modulate_backup: Color = Color(1, 1, 1, 1)
var counter_slow_canvas_modulate_applied: bool = false
var counter_slow_canvas_modulate_tween: Tween = null
...
var horizontal_input_priority: int = 0
...
func _physics_process(delta):
	...
	var current_real_time: float = Time.get_unix_time_from_system()
	if counter_slow_compensation_active:
		if current_real_time >= counter_slow_compensation_end_time:
	...
	if counter_slow_compensation_active:
		counter_move_scale = 1.0 / maxf(Engine.time_scale, 0.01)
...
func _on_dash_duration_timeout():
	...
	if was_gliding_before_dash:
		...
		var move_input = get_resolved_horizontal_input()
		if move_input == 0:
...
func _on_backstep_unlocked():
	backstep_unlocked = true

func enable_counter_slow_compensation(real_duration: float) -> void:
...
func enable_counter_slow_invincibility(real_duration: float) -> void:
	if real_duration <= 0.0:
		counter_slow_invincible_active = false
...
func _update_counter_slow_visuals(active: bool) -> void:
	if active == counter_slow_visual_overlay_active:
		return

	counter_slow_visual_overlay_active = active
	if active:
		var darken_alpha: float = clampf(backstep_counter_darken_alpha, 0.0, 0.85)
		var darken_factor: float = maxf(1.0 - darken_alpha, 0.15)
		var fade_duration: float = maxf(backstep_counter_darken_fade_duration, 0.0)
		var room_container := _resolve_runtime_room_container()
		if is_instance_valid(room_container):
			if not counter_slow_room_modulate_applied or counter_slow_room_modulate_target != room_container:
				counter_slow_room_modulate_target = room_container
				counter_slow_room_modulate_backup = room_container.modulate
				counter_slow_room_modulate_applied = true
			var room_base_color: Color = counter_slow_room_modulate_backup
			var room_target_color := Color(room_base_color.r * darken_factor, room_base_color.g * darken_factor, room_base_color.b * darken_factor, room_base_color.a)
			if counter_slow_room_modulate_tween and counter_slow_room_modulate_tween.is_valid():
				counter_slow_room_modulate_tween.kill()
			if fade_duration > 0.0:
				counter_slow_room_modulate_tween = create_tween()
				counter_slow_room_modulate_tween.set_trans(Tween.TRANS_SINE)
				counter_slow_room_modulate_tween.set_ease(Tween.EASE_OUT)
				counter_slow_room_modulate_tween.tween_property(room_container, "modulate", room_target_color, fade_duration)
			else:
				room_container.modulate = room_target_color
		elif is_instance_valid(canvas_modulate):
			if not counter_slow_canvas_modulate_applied:
				counter_slow_canvas_modulate_backup = canvas_modulate.color
				counter_slow_canvas_modulate_applied = true
			var base_color: Color = counter_slow_canvas_modulate_backup
			var canvas_target_color := Color(base_color.r * darken_factor, base_color.g * darken_factor, base_color.b * darken_factor, base_color.a)
			if counter_slow_canvas_modulate_tween and counter_slow_canvas_modulate_tween.is_valid():
				counter_slow_canvas_modulate_tween.kill()
			if fade_duration > 0.0:
				counter_slow_canvas_modulate_tween = create_tween()
				counter_slow_canvas_modulate_tween.set_trans(Tween.TRANS_SINE)
				counter_slow_canvas_modulate_tween.set_ease(Tween.EASE_OUT)
				counter_slow_canvas_modulate_tween.tween_property(canvas_modulate, "color", canvas_target_color, fade_duration)
			else:
				canvas_modulate.color = canvas_target_color
		elif typeof(DarkOverlay) != TYPE_NIL and is_instance_valid(DarkOverlay):
			if DarkOverlay.has_method("set_overlay_alpha"):
				DarkOverlay.set_overlay_alpha(backstep_counter_darken_alpha)
			if DarkOverlay.has_method("show_overlay"):
				DarkOverlay.show_overlay()
		return

	if counter_slow_room_modulate_applied and is_instance_valid(counter_slow_room_modulate_target):
		var restore_room_color: Color = counter_slow_room_modulate_backup
		if counter_slow_room_modulate_tween and counter_slow_room_modulate_tween.is_valid():
			counter_slow_room_modulate_tween.kill()
		if backstep_counter_darken_fade_duration > 0.0:
			counter_slow_room_modulate_tween = create_tween()
			counter_slow_room_modulate_tween.set_trans(Tween.TRANS_SINE)
			counter_slow_room_modulate_tween.set_ease(Tween.EASE_OUT)
			counter_slow_room_modulate_tween.tween_property(counter_slow_room_modulate_target, "modulate", restore_room_color, backstep_counter_darken_fade_duration)
		else:
			counter_slow_room_modulate_target.modulate = restore_room_color
		counter_slow_room_modulate_applied = false
		counter_slow_room_modulate_target = null
	if counter_slow_canvas_modulate_applied and is_instance_valid(canvas_modulate):
		var restore_canvas_color: Color = counter_slow_canvas_modulate_backup
		if counter_slow_canvas_modulate_tween and counter_slow_canvas_modulate_tween.is_valid():
			counter_slow_canvas_modulate_tween.kill()
		if backstep_counter_darken_fade_duration > 0.0:
			counter_slow_canvas_modulate_tween = create_tween()
			counter_slow_canvas_modulate_tween.set_trans(Tween.TRANS_SINE)
			counter_slow_canvas_modulate_tween.set_ease(Tween.EASE_OUT)
			counter_slow_canvas_modulate_tween.tween_property(canvas_modulate, "color", restore_canvas_color, backstep_counter_darken_fade_duration)
		else:
			canvas_modulate.color = restore_canvas_color
		counter_slow_canvas_modulate_applied = false
	elif typeof(DarkOverlay) != TYPE_NIL and is_instance_valid(DarkOverlay):
		if DarkOverlay.has_method("hide_overlay"):
			DarkOverlay.hide_overlay()

func _resolve_runtime_room_container() -> CanvasItem:
	var current_scene := get_tree().current_scene
	if current_scene:
		var room_container := current_scene.get_node_or_null("RoomContainer")
		if room_container and room_container is CanvasItem:
...
func consume_backstep_counter() -> void:
	backstep_counter_consumed = true
...
func get_resolved_horizontal_input() -> float:
	var left_pressed: bool = Input.is_action_pressed("left")
	var right_pressed: bool = Input.is_action_pressed("right")
	var left_just_pressed: bool = Input.is_action_just_pressed("left")
	var right_just_pressed: bool = Input.is_action_just_pressed("right")

	if left_just_pressed:
		horizontal_input_priority = -1
	if right_just_pressed:
		horizontal_input_priority = 1

	if left_pressed and right_pressed:
		if horizontal_input_priority == 0:
			horizontal_input_priority = -1 if is_facing_right else 1
		return float(horizontal_input_priority)
	if left_pressed:
		horizontal_input_priority = -1
		return -1.0
	if right_pressed:
