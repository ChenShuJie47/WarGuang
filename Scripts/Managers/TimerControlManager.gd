...
func slow_motion(preset: String) -> void:
	var config: Dictionary = get_slow_motion_preset_config(preset)
	if config.is_empty():
		push_error("TimerControlManager: 无效的慢动作预设 ", preset)
		return
	start_slow_motion(float(config.get("duration", 0.0)), float(config.get("time_scale", 1.0)), float(config.get("transition", 0.0)))

func get_slow_motion_preset_config(preset: String) -> Dictionary:
	match preset:
		"light":
			return {
				"duration": slow_light_duration,
				"time_scale": slow_light_time_scale,
				"transition": slow_light_transition
			}
		"medium":
			return {
				"duration": slow_medium_duration,
				"time_scale": slow_medium_time_scale,
				"transition": slow_medium_transition
			}
		"heavy":
			return {
				"duration": slow_heavy_duration,
				"time_scale": slow_heavy_time_scale,
				"transition": slow_heavy_transition
			}
		"extreme":
			return {
				"duration": slow_extreme_duration,
				"time_scale": slow_extreme_time_scale,
				"transition": slow_extreme_transition
			}
		_:
			return {}

func start_counter_slow_motion_for_player(player: Node, preset: String = "light") -> void:
	var config: Dictionary = get_slow_motion_preset_config(preset)
	if config.is_empty():
		return
	var duration: float = float(config.get("duration", 0.0))
	var time_scale: float = maxf(float(config.get("time_scale", 1.0)), 0.01)
	var transition: float = float(config.get("transition", 0.0))
	var real_time_duration: float = (duration + transition) / time_scale
...
func stop_slow_motion() -> void:
	...
	var restore_transition: float = maxf(_slow_motion_transition, 0.0)
	if restore_transition <= 0.0:
		Engine.time_scale = 1.0
		return

	# 使用与进入慢动作相同的 transition，避免固定 0.2 秒带来的体感与时序不一致。
	var tween = create_tween()
	tween.tween_method(
		func(value): Engine.time_scale = value,
		Engine.time_scale,
