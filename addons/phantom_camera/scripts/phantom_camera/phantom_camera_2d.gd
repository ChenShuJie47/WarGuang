...
## becoming active. The argut is the [param PhantomCamera2D] that interrupted
## the tween.
			...
			if not Engine.is_editor_hint():
				if not _is_active:
					...
					if framed_side_offset != Vector2.ZERO:
						...
						# Keep follow target in sync when inside dead zone (also fixes teleport_position() stale target)
						_follow_target_position = _transform_output.origin
						return
			else:
				_follow_target_position = _get_target_position_offset()
...
func _check_physics_body(target: Node2D) -> void:
	if target is PhysicsBody2D:
		...
		Engine.get_version_info().minor < 3:
			...
			push_warning("Physics Interpolation is disabled in the Project Settings, recommend enabling it to smooth out physics-based camera movement")
			print_rich("This tip can be disabled from within [code]Project Settings / Phantom Camera / Tips / Show Jitter Tips[/code]")
...
func teleport_position() -> void:
	...
	if follow_mode == FollowMode.FRAMED:
		_follow_framed_offset = Vector2.ZERO
	_set_follow_position()
